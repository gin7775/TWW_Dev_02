...
public class Puppet : MonoBehaviour, IEnemyFreezable
{
    ...
    public float knockbackDuration = 0.3f; // DuraciÛn del retroceso
    public float angulo;
    ...
    // MÈtodo para descongelar al enemigo
    private IEnumerator Unfreeze(float freezeDuration)
    {
        yield return new WaitForSeconds(freezeDuration);
        agent.speed = 2;
        // Reactivar la animaciÛn ajustando la velocidad de vuelta a la normalidad
    ...
    // Instanciar el VFX de sangre en la direcciÛn opuesta
    ...
    private void ApplyKnockback()
    {
        ...
        {
            // Reactivar el NavMeshAgent despuÈs del knockback
            agent.enabled = true;

            // Sincronizar la posiciÛn del NavMeshAgent con la nueva posiciÛn tras el knockback
    ...
    IEnumerator UpdateSliderOrientation()
    {
        while (true)
        {
            // Copia la rotaciÛn de la c·mara, pero mantiene el `Slider` orientado horizontalmente respecto al suelo.
            Quaternion cameraRotation = Camera.main.transform.rotation;
            cameraRotation.x = 0; // Neutraliza la rotaciÛn en X
            cameraRotation.z = 0; // Neutraliza la rotaciÛn en Z
